import numpy as np
import time

# Anscombe datasets
datasets = [
    {'x': [10, 8, 13, 9, 11, 14, 6, 4, 12, 7, 5], 'y': [8.04, 6.95, 7.58, 8.81, 8.33, 9.96, 7.24, 4.26, 10.84, 4.82, 5.68]},
    {'x': [10, 8, 13, 9, 11, 14, 6, 4, 12, 7, 5], 'y': [9.14, 8.14, 8.74, 8.77, 9.26, 8.1, 6.13, 3.1, 9.13, 7.26, 4.74]},
    {'x': [10, 8, 13, 9, 11, 14, 6, 4, 12, 7, 5], 'y': [7.46, 6.77, 12.74, 7.11, 7.81, 8.84, 6.08, 5.39, 8.15, 6.42, 5.73]},
    {'x': [8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 19], 'y': [6.58, 5.76, 7.71, 8.84, 8.47, 7.04, 5.25, 5.56, 7.91, 6.89, 12.50]},
]

# Function to compute linear regression coefficients (slope and intercept)
def linear_regression(x, y):
    x = np.array(x)
    y = np.array(y)
    
    # Calculate the slope (m) and intercept (b) using the formula
    m, b = np.polyfit(x, y, 1)  # Fit a line to the data (1st-degree polynomial)
    
    return m, b

# Start time
start_time = time.time()

# Loop through each dataset and calculate the regression coefficients
for i, dataset in enumerate(datasets, 1):
    x = dataset['x']
    y = dataset['y']
    
    slope, intercept = linear_regression(x, y)
    print(f"Dataset {i}: Slope = {slope:.2f}, Intercept = {intercept:.2f}")

# End time
end_time = time.time()

# Calculate and print the elapsed time
execution_time = end_time - start_time
print(f"Execution time: {execution_time * 1_000_000:.0f} µs")
